...
if (newStatus === "Delivered") {
  ...
  newPaymentStatus = order.paymentStatus === "Paid" ? "Refunded" : "Unpaid";
} else if (newStatus === "Pending") {
  newPaymentStatus = "Unpaid";
} else {
  // For other statuses like "Processing", "Shipped"
  newPaymentStatus = order.paymentStatus;
}


    const { data } = await axios.put(`http://localhost:5000/api/orders/${orderId}`, {
      status: newStatus,
    ...
    setSelectedOrder(prev =>
      prev ? { ...prev, status: newStatus, paymentStatus: newPaymentStatus } : null
    );
